...
Use `v2/ornaments/puzzle-transition.png` with empty `alt` inside the hidden
decorative wrapper, not the `v2/marks/puzzles.png` masthead logo. The 128px RGBA
asset is generated by `tools/make_ornaments.py`: a hollow lozenge with two points,
not another subject illustration. Layout and orientation stay local. Jekyll's
separate `data-image-decoration` opt-out keeps its image-caption processor from
framing a decorative mark. An existing layout-specific placement is not evidence
that the main-unit boundary has been verified.

The public archive's `.puzzle-sheet` wraps the entire main segment, with the
divider as its top edge and `.puzzle-sheet-body` supplying a modest responsive
inset. One thin square frame and a neutral surface ground the grid; no shadows,
internal scrolling, image filters, or added headings. Examples remain outside.
The frame includes paired grids, post-grid solve links, notes, and reference
...
wrappers. Only this verified run uses flattened paragraph layout. Do not apply
`display: contents` to arbitrary puzzle prose. The two image-first exceptions keep
